...
# ============================================================
# LimitedDict với reentrant lock + OrderedDict copy-on-write
# ============================================================
...
class LimitedDict:
    ...
    Optimization: dùng RLock để giảm contention khi read lồng read,
    và snapshot() trả về list copy để display loop không giữ lock.
    """
    ...
    def snapshot(self) -> list:
        """Trả về list snapshot (key, value) - không giữ lock lâu."""
        with self._lock:
...
class PacketFilter:
    ...
    @staticmethod
    def _parse_cidr(cidr: str) -> Optional[Tuple[str, int]]:
        """Parse CIDR như '192.168.0.0/16' -> (base_ip_int, prefix_len)."""
        try:
            if '/' not in cidr:
                return cidr, 32
            ip_part, prefix = cidr.split('/', 1)
            prefix_len = int(prefix)
            return ip_part, prefix_len
        except Exception:
            return None
    ...
    def _ip_in_network(self, ip: str) -> bool:
        ...
        if ip_int is None:
            return False
        base_str, prefix = self._network
        base_int = self._ip_to_int(base_str)
        if base_int is None:
            return False
        if prefix == 0:
            return True
...
class PacketListView:
    ...
    def __init__(
        ...
        self.packet_map = LimitedDict(maxsize=cache_size)
