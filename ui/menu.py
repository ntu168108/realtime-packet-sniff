...
def input_choice(prompt: str, choices: List[str], default: str = '') -> str:
    ...
    while True:
        ...
        if result in [c.upper() for c in choices]:
            return result
        print(error(f"Luạ chọn không hợp lệ! Chọn: {', '.join(choices)}"))
...
class MainMenu:
    ...
    def __init__(
        ...
        on_live_mode: Optional[Callable] = None,
        on_quick_stats: Optional[Callable] = None,
    ):
        self.on_quick_capture = on_quick_capture
        self.on_advanced_capture = on_advanced_capture
        self.on_open_pcap = on_open_pcap
        self.on_settings = on_settings
        self.on_live_mode = on_live_mode
        self.on_quick_stats = on_quick_stats
    ...
    def show(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                self.open_pcap_menu()
            elif choice == '4' and self.on_live_mode:
                self.live_mode_menu()
            elif choice == '5' and self.on_quick_stats:
                self.quick_stats_menu()
            elif choice == '6':
                self.settings_menu()
            elif choice == '7':
                self.show_help()
    ...
    # ------------------------------------------------------------
    # NEW: Live mode menu - stream JSON ra stdout (no TUI)
    # ------------------------------------------------------------

    def live_mode_menu(self):
        """Live mode menu - stream JSON packets ra stdout (no TUI)."""
        if not self.on_live_mode:
            print(error("Live mode không khả dụng"))
            wait_for_key()
            return

        clear_screen()
        print_header(" LIVE MODE (JSON STREAM) ", '═')
        print()
        print(dim("  Stream mỗi packet ra stdout dưới dạng JSON line."))
        print(dim("  Phù hợp để pipe vào các tool khác (jq, file, ...)."))
        print(dim("  Ví dụ: sniff | jq '.protocol_name'"))
        print()

        # Interface selection
        interfaces = get_interfaces_with_info()
        if not interfaces:
            print(error("Không tìm thấy card mạng!"))
            wait_for_key()
            return

        for i, iface in enumerate(interfaces, 1):
            status = green("UP") if iface['up'] else red("DOWN")
            ip = iface['ip'] if iface['ip'] else dim("no-ip")
            print(f"  [{i}] {bold(iface['name'])} - {status} - {ip}")
        ...
        # Optional filter
        print()
        filter_expr = input_with_default("BPF filter (optional)", self.settings.get('filter_expr', ''))

        print()
        print_divider()
        print()
        print(bold("Bắt đầu Live mode?"))
        print(dim(f"  Interface: {selected_iface}"))
        if filter_expr:
            print(dim(f"  Filter:    {filter_expr}"))
        print()
        print(dim("  Nhấn Ctrl+C để dừng."))
        print()

        if not confirm("Bắt đầu?", default=True):
            return
    ...
    # ------------------------------------------------------------
    # NEW: Quick stats - capture 30s và in summary
    # ------------------------------------------------------------

    def quick_stats_menu(self):
        """Quick stats menu - capture trong 30s rồi in summary."""
        if not self.on_quick_stats:
            print(error("Quick stats không khả dụng"))
            wait_for_key()
            return
        ...
        print()
        print(dim("  Capture trong khoảng thời gian nhất định rồi in"))
        print(dim("  summary gồm: protocol breakdown, top talkers, pps/bps."))
        print()

        # Duration
        duration = input_with_default(
            "Thời gian capture (giây)",
            str(self.settings.get('quick_stats_duration', 30))
        )
        try:
        ...
        print_divider()
        print()
        print(bold(f"Sẽ capture trên {selected_iface} trong {duration}s"))
        print()
