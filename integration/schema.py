...
def column_default(ch_type: str) -> str:
    """Return a CH DEFAULT literal suitable for a missing-per-family column.

    Used in the DDL so missing columns store a sensible default instead of
    the column default of the column's data type (which would be 0 / '').
    """
    t = ch_type.lower()
    if t.startswith("int") or t.startswith("uint") or t.startswith("float"):
        return "0"
    if t == "string":
        return "''"
    return "0"
