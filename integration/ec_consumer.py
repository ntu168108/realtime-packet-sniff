...
def _stub_sink_factory():
    """A throwaway sink that just counts rows per family.

    Used for the real-integration check before Phase 3 (ClickHouseSink) lands.
    Activated by env var EC_CONSUMER_SINK=stub.
    """
    class _Stub:
        def __init__(self):
            self.counts = {}

        def insert_family(self, family, csv_path, meta):
            try:
                with open(csv_path, encoding="utf-8", errors="ignore") as f:
                    n = sum(1 for _ in f) - 1  # minus header
            except OSError:
                n = 0
            self.counts[family] = self.counts.get(family, 0) + max(n, 0)
            logger.info("stub sink: family=%s csv=%s rows=%d", family, csv_path, n)
            return max(n, 0)
...
def main():
    ...
    # Sink selection: stub for now (Phase 3 brings ClickHouseSink).
    if os.environ.get("EC_CONSUMER_SINK") == "stub":
        sink = _stub_sink_factory()
    else:
        try:
            from .clickhouse_sink import ClickHouseSink  # type: ignore

            sink = ClickHouseSink(cfg["clickhouse"])
        except ImportError:
            logger.warning(
                "ClickHouseSink not available; falling back to stub sink. "
                "Set EC_CONSUMER_SINK=stub to silence this warning."
            )
            sink = _stub_sink_factory()
