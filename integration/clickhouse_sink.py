...
def _cast_cell(value: Any, ch_type: str) -> Any:
    ...
    Rules:
      - NaN / None / missing -> type-appropriate default (0 for numerics, '' for String)
      - numeric columns -> float / int
      - String / LowCardinality(String) -> str (truncated to None-safe)
...
def _resolve_ts(row: pd.Series, now: datetime, segment_fallback: float | None = None) -> datetime:
    ...
    if "ltime" in row.index:
        try:
            ...
            pass
    if segment_fallback is not None and segment_fallback > 0:
        try:
            return datetime.fromtimestamp(float(segment_fallback), tz=timezone.utc)
        except (TypeError, ValueError, OSError):
            pass
    return now
