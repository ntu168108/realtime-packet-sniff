...
def _evaluate_rule_vectorized(rule: dict, df: pd.DataFrame, missing_logged: set) -> np.ndarray:
    ...
    if op == "==":
        return col == threshold
    raise ValueError(f"Unknown op in rule: {op!r}")
...
def downcast_integers(df: pd.DataFrame, int_columns: list) -> pd.DataFrame:
    """Downcast integer columns to int16/int32/int64 based on range.

    Skips columns with NaN values (cannot downcast nullable int cleanly).
    Logs warning if column has NaN.
    """
    df = df.copy()
    df_cols_lower = {c.lower(): c for c in df.columns}
    ...
    for col in int_columns:
        ...
        if col_orig is None:
            continue
        if df[col_orig].isna().any():
            logger.warning(f"Column '{col_orig}' contains NaN; skipping downcast")
            continue
        try:
            col_min = df[col_orig].min()
            col_max = df[col_orig].max()
            if col_min >= np.iinfo(np.int16).min and col_max <= np.iinfo(np.int16).max:
                df[col_orig] = df[col_orig].astype(np.int16)
            elif col_min >= np.iinfo(np.int32).min and col_max <= np.iinfo(np.int32).max:
                df[col_orig] = df[col_orig].astype(np.int32)
            else:
                df[col_orig] = df[col_orig].astype(np.int64)
