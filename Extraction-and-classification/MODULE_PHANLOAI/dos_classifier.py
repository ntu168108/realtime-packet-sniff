...
def _precompute_conditions(
    ...
    is_multicast = np.array([
        str(ip).startswith(('224.', '225.', '226.', '227.', '228.', '229.',
...
# ---------------------------------------------------------------------------
# Engine tính điểm cho 1 bộ scoring
# ---------------------------------------------------------------------------
def _compute_subtype_score(
    conds: dict,
    proto_mask: np.ndarray,
    score_rules: dict,
) -> np.ndarray:
    """
    Tính điểm cho 1 loại tấn công cụ thể, chỉ áp dụng cho flow
    thuộc đúng protocol mask.

    Args:
        conds: Dict boolean arrays từ _precompute_conditions.
        proto_mask: Boolean array xác định flow thuộc proto nào.
        score_rules: Dict {rule_name: score_value}.

    Returns:
        numpy array float — điểm cho từng flow (0 nếu không thuộc proto).
    """
    n = len(proto_mask)
    scores = np.zeros(n, dtype=np.float64)

    for rule_name, score_value in score_rules.items():
        if rule_name in conds:
            # Chỉ cộng điểm cho flow thuộc đúng protocol
            scores += np.where(proto_mask & conds[rule_name], score_value, 0)
...
def evaluate_dos_scores(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    syn_scores = _compute_subtype_score(
        conds, conds['is_tcp'], config['syn_flood']['scores']
    )

    # ----- UDP Flood Score (chỉ cho UDP nghi ngờ — loại trừ mDNS/DHCP/broadcast) -----
    udp_scores = _compute_subtype_score(
        conds, conds['is_udp_suspect'], config['udp_flood']['scores']
    )

    # ----- ICMP Flood Score (chỉ cho ICMP) -----
    icmp_scores = _compute_subtype_score(
        conds, conds['is_icmp'], config['icmp_flood']['scores']
    )
...
def classify_dos_subtype(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    # numpy.select: ưu tiên theo điểm cao nhất
    # Vì mỗi flow chỉ thuộc 1 proto nên thực tế không overlap,
    # nhưng vẫn đặt thứ tự ưu tiên rõ ràng.
    conditions = [is_syn, is_udp, is_icmp]
    choices    = ['SYN Flood', 'UDP Flood', 'ICMP Flood']

    attack_types = np.select(conditions, choices, default='')

    # Gán kết quả — dòng không vượt ngưỡng giữ NaN
    df['attack_type'] = attack_types
    df.loc[df['attack_type'] == '', 'attack_type'] = np.nan
...
def main() -> None:
    ...
    if not args.skip_filter:
        ...
        if name_part_raw.endswith("_dos_features"):
            ...
            filter_dos_features(csv_path_raw, filtered_output)
