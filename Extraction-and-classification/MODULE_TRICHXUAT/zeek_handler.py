...
# Mapping: ten truong trong conn.log -> ten cot trong CSV
CONNLOG_FIELDS = {
    "id.orig_h":    "id.orig_h",
    "id.resp_h":    "id.resp_h",
    "id.orig_p":    "id.orig_p",
    ...
    "http_method":   "http_method",   # gia tri HTTP method -> key cho ct_flw_http_mthd (Alg 3.4)
    "is_ftp_login":  "is_ftp_login",
    "ftp_cmd":       "ftp_cmd",       # gia tri FTP command -> key cho ct_ftp_cmd (Alg 3.6)
}
...
def run_zeek(pcap_file: str, work_dir: str) -> str:
    ...
    wanted_fields = list(CONNLOG_FIELDS.keys())
    available = {}
    ...
    csv_columns = [CONNLOG_FIELDS[f] for f in wanted_fields if f in available or f in new_fields]
    ...
    with open(log_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line.startswith("#fields"):
                header_fields = line.split("\t")[1:]  # Bo "#fields"
