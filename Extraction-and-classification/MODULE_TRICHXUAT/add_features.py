...
def parse_dtcpb(val) -> int:
    """
    Parse Destination TCP base sequence number (dtcpb).
    Mặc định trả về 0 nếu có lỗi, NaN, trống hoặc kết nối bị REJ/RST.
    """
    try:
...
def parse_service(val) -> str:
    """
    Parse Application Layer Protocol (service).
    Mặc định trả về '-' nếu trống, lỗi, hoặc không có payload do RST sớm.
    """
    try:
        if pd.isna(val):
            return "-"
        val_str = str(val).strip().lower()
        if val_str in ("", "nan", "None", "-", "-1", "unknown"):
            return "-"
        return val_str
    except Exception:
        return "-"
...
def main():
    ...
    logger.info("  Tinh toan cot ct_dst_ltm...")
    df["ct_dst_ltm"] = compute_sliding_counts(dstip_list, window_size)

    logger.info("  Tinh toan cot ct_src_ltm...")
    df["ct_src_ltm"] = compute_sliding_counts(srcip_list, window_size)

    logger.info("  Tinh toan cot ct_srv_dst...")
    df["ct_srv_dst"] = compute_sliding_counts(srv_dst, window_size)

    logger.info("  Tinh toan cot ct_srv_src...")
    df["ct_srv_src"] = compute_sliding_counts(srv_src, window_size)

    logger.info("  Tinh toan cot ct_src_dport_ltm...")
    df["ct_src_dport_ltm"] = compute_sliding_counts(src_dport, window_size)

    logger.info("  Tinh toan cot ct_dst_sport_ltm...")
    df["ct_dst_sport_ltm"] = compute_sliding_counts(dst_sport, window_size)

    logger.info("  Tinh toan cot ct_dst_src_ltm...")
    df["ct_dst_src_ltm"] = compute_sliding_counts(src_dst, window_size)

    # 3.4 ct_flw_http_mthd (Alg 3.4) + ct_ftp_cmd (Alg 3.6)
    # Input tuple gốc: (srcip, dstip, sport, dsport, method/command). Vì vậy
    # khóa = 4-tuple + GIÁ TRỊ method / command. Gate = giá trị có mặt (khác rỗng):
    # flow không phải HTTP/FTP (method/command rỗng) -> luôn nhận 0 (nhánh else).
    base_tuple = [
        f"{src}_{dst}_{sp}_{dp}"
        for src, dst, sp, dp in zip(srcip_list, dstip_list, sport_list, dport_list)
    ]

    logger.info("  Tinh toan cot ct_flw_http_mthd...")
    ...
    logger.info("  Tinh toan cot ct_ftp_cmd...")
