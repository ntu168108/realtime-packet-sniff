...
def _config_yaml_path() -> Path:
    """Resolve the absolute path to config.yaml — try CWD first, then repo root.

    Matches the resolution order used by the `_on_startup` handler (lines
    495-504): the systemd unit sets WorkingDirectory to the repo root, but
    tests may run from elsewhere, so we mirror that logic here.
    """
...
def _interface_ipv4(iface: str) -> Optional[str]:
    ...
    try:
        ...
        for a in addrs:
            # psutil.AF_INET == socket.AF_INET (== 2); compare by value
            if getattr(a, "family", None) == 2 and a.address:
                return a.address
