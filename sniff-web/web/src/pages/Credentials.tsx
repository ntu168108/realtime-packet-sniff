    ...
    <div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <table className="cred-table">
          ...
          <tbody>
            {rows.map(({ label, data }) => {
              const urlKey = `${label}-url`;
              const userKey = `${label}-user`;
              const passKey = `${label}-pass`;
              return (
                <tr key={label}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <button
                        type="button"
                        className={`copy-btn ${copied === urlKey ? 'copied' : ''}`}
                        onClick={() => copy(urlKey, data.url)}
                      >
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      {data.username && (
                        <button
                          type="button"
                          className={`copy-btn ${copied === userKey ? 'copied' : ''}`}
                          onClick={() => copy(userKey, data.username)}
                        >
                          {copied === userKey ? 'copied' : 'copy'}
                        </button>
                      )}
                    </div>
                  </td>
                  <td>
                    {data.password ? (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        ...
                        <button
                          type="button"
                          className={`copy-btn ${copied === passKey ? 'copied' : ''}`}
                          onClick={() => copy(passKey, data.password!)}
                        >
