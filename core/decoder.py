...
class PacketInfo:
    ...
    # Optional cached decode (để tránh decode 2 lần ở UI list view)
    # Chỉ filled khi decoder chạy; mặc định None
    cached_protocol: Optional[str] = None
...
def decode_packet_scapy(pkt, deep: bool = False) -> DecodedPacket:
    """
    Decode using Scapy packet object - alternative method.
    Cùng signature với decode_packet, có cùng deep flag.
    """
    try:
        ...
        return decode_packet(bytes(pkt), deep=deep)
    ...
    if IP in pkt:
        ...
        transport = result.protocol_name
    ...
    if IPv6 in pkt:
        result.ipv6 = IPv6Header(
            ...
            src_ip=pkt[IPv6].src,
            dst_ip=pkt[IPv6].dst,
            next_header_name=PROTO_NAMES.get(pkt[IPv6].nh, str(pkt[IPv6].nh))
        )
    ...
    if TCP in pkt:
        ...
        src_port, dst_port = result.src_port, result.dst_port
