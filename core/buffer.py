...
class BoundedRingBuffer(RingBuffer):
    """
    Ring buffer với block-on-full semantics (backpressure).
    Khi đầy, put_nowait sẽ drop thay vì block, nhưng expose
    thêm tín hiệu pressure để caller biết khi nào nên giảm tải.
    """

    @property
    def pressure(self) -> float:
        """Mức đầy từ 0.0 -> 1.0. Dùng để trigger backpressure policy."""
        if self._maxlen <= 0:
            return 0.0
