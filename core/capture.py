...
from .constants import (
    ...
    DEFAULT_RING_BUFFER_SIZE, DEFAULT_BPF_VALIDATE_TIMEOUT,
    DEFAULT_HIGH_PRESSURE_THRESHOLD, MAX_DISPLAY_FILTER_LEN,
    DEFAULT_PAYLOAD_SNIPPET_BYTES, DROP_STATS_UPDATE_INTERVAL,
...
class CaptureEngine:
    ...
    def __init__(
        ...
        on_packet_written: Optional[Callable[[PacketInfo], None]] = None,
        on_drop: Optional[Callable[[str, int], None]] = None,
        count: int = 0,
        enable_deep_decode: bool = False,
    ):
        """
        Args:
            ...
            on_packet_written: Hook sau khi ghi PCAP thành công
            on_drop: Hook khi có drop (reason, count)
            count: Stop sau N packets (0 = unlimited)
            enable_deep_decode: Bật L7 deep decode (DNS/HTTP/TLS/...)
        """
        ...
        self.on_packet_written = on_packet_written
        self.on_drop = on_drop
        self.count = count
        self.enable_deep_decode = enable_deep_decode
    ...
    def setup(self):
        ...
        try:
            ...
            # Validate BPF filter nếu có (fail fast)
            if self.bpf_filter:
                self._validate_bpf_filter(self.bpf_filter)
    ...
    def _validate_bpf_filter(self, bpf: str):
        """
        Validate BPF filter expression. Modern scapy no longer ships its
        own BPF compiler; the kernel validates the filter at sniff() time.
        We do basic syntax sanity here (non-empty, no obvious garbage) and
        rely on the kernel to reject truly invalid filters.
        """
        try:
            from scapy.all import compile_filter  # type: ignore
            try:
                compile_filter(bpf)
                logger.debug(f"BPF filter OK: {bpf!r}")
                return
            except Exception:
                # scapy's own compiler disagrees — log and fall through to
                # the kernel-side validation below.
                logger.debug(
                    "scapy compile_filter rejected %r; deferring to kernel",
                    bpf,
                )
        except ImportError:
            # scapy >= 2.5 removed compile_filter; just defer to kernel.
            pass
        # Kernel-side validation surface: sniff() will raise OSError/
        # Scapy_Exception on bad filter; nothing more we can do statically
        # without re-implementing the BPF compiler. Log only.
            ...
            for pkt_info in batch:
                ...
                if self.rotator:
                    try:
                        ...
                        if self.on_packet_written:
                            try:
                                self.on_packet_written(pkt_info)
                            except Exception:
                                pass
                    except Exception as e:
    ...
    def _update_proto_stats(self, pkt_info: PacketInfo):
        """
        Cập nhật per-protocol counter. Cheap nếu dùng cached_protocol.
        Nếu chưa cache, sniff protocol từ data (L2/L3/L4 only, không deep).
        """
        proto = pkt_info.cached_protocol
        if not proto:
            proto = self._sniff_protocol(pkt_info.data)
            pkt_info.cached_protocol = proto
