...
class HourlyRotator:
    ...
    def write_packet(self, ts_sec: int, ts_usec: int, data: bytes, origlen: int = None):
        ...
        if writer is None:
            ...
            # Check size rotation (cheap, dùng cached counter)
            if self.max_file_size > 0 and writer.byte_count >= self.max_file_size:
                with self._lock:
                    # Re-check sau khi acquire lock
                    if self._current_writer is writer and writer.byte_count >= self.max_file_size:
                        self._do_rotate(datetime.now())
                        writer = self._current_writer
            else:
                # Check time rotation
                now = datetime.now()
                if self._next_rotate_time and now >= self._next_rotate_time:
                    with self._lock:
                        if self._current_writer is writer and self._next_rotate_time and now >= self._next_rotate_time:
                            self._do_rotate(now)
                            writer = self._current_writer
