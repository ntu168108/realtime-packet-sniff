...
class PcapRingWriter:
    """
    Wrapper quanh nhiều PcapWriter instances dùng cho concurrent writes
    (nếu sau này cần tách thread write riêng).

    Hiện tại đơn giản: giữ 1 writer tại 1 thời điểm, swap khi cần.
    """

    def __init__(self, *writers: PcapWriter):
    ...
    def write_packet_info(self, pkt_info: PacketInfo):
        if not self._writers:
            return
        with self._lock:
            w = self._writers[self._idx % len(self._writers)]
        w.write_packet_info(pkt_info)

    def rotate_to(self, writer: PcapWriter):
        """Swap to a new writer"""
        with self._lock:
            self._writers.append(writer)
            self._idx = len(self._writers) - 1
